...
class RSNA24Dataset_Cls_V2_Multi_Axial(Dataset):
    def __init__(self,
        ...
        self.keypoints_infos = pickle.load(
            open(f'{data_root}/pred_keypoints/v20_sag_2d_keypoints_center_slice_v2.pkl', 'rb'))

        self.t1_keypoints_infos_3d = pickle.load(
            open(f'{data_root}/v20_sag_T1_3d_keypoints_en5_resnet34d.pkl', 'rb'))
    ...
    def __getitem__(self, idx):
        ...
        if 'Sagittal T1' not in des_to_sid.keys():
            ...
            s_t1 = self.load_volume(fns, index=index)

            t1_pred_keypoints = self.t1_keypoints_infos_3d[study_id][sid].reshape(10, 3).astype(np.float32)
            #print('t1_pred_keypoints: ', t1_pred_keypoints)
            #t1_pred_keypoints[:, :2] = G_IMG_SIZE * t1_pred_keypoints[:, :2]
