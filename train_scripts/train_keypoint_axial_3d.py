...
def get_args():
    ...
    parser.add_argument('--save_dir', type=str, default='./wkdir/keypoint/axial_3d_fix/')
...
N_FOLDS = 8
TRAIN_FOLDS_LIST = [0, 1, 2]  # keypoint use 3 fold is enough
EPOCHS = 30
...
if args.only_val != 1:
    ...
    for fold, (trn_idx, val_idx) in enumerate(skf.split(range(len(df)))):
        ...
        model = RSNA24Model_Keypoint_3D_Axial(MODEL_NAME, pretrained=True)
        model.to(device)

        # model.load_state_dict(
        #     torch.load(f'./wkdir/keypoint/axial_3d/densenet161_lr_0.0006/best_fold_{fold}_ema.pt'))

        train_ds = RSNA24Dataset_KeyPoint_Axial_3D(
            data_root,
            aux_info,
        ...
        valid_ds = RSNA24Dataset_KeyPoint_Axial_3D(
            data_root,
            aux_info,
