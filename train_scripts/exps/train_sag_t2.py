...
def get_args():
    ...
    parser.add_argument('--save_dir', type=str, default='./wkdir/v20/sag_t2/')
...
if with_level_lstm:
    OUTPUT_DIR = OUTPUT_DIR + '_level_lstm'
...
if True:#args.only_val == 1:
    ...
    for fold, (trn_idx, val_idx) in enumerate(skf.split(range(len(df)))):
        ...
        # ]
        # df_valid = df_valid[~df_valid['study_id'].isin(without)]
                ...
                for idx, tensor_dict in enumerate(pbar):
                    ...
                    with autocast:
                        ...
                        for col in range(N_LABELS):
                            ...
                            labels.append(gt.cpu())
                        #bs, _ = ye.shape
                        #fold_preds.append(ye.reshape(bs, -1, 5, 3).cpu().float())
        # fold_preds = torch.cat(fold_preds, dim=0)
        #
        # labels_tmp = df_valid.iloc[:, 1:].values
        # labels_tmp = torch.from_numpy(labels_tmp)
        # t2_labels = labels_tmp[:, :5].reshape(-1, 1, 5).reshape(-1)
        # t2_pred = fold_preds[:, 0:1, :, :].reshape(-1, 3)
    ...
    y_prede = y_prede_np#np.concatenate([y_pred_nan, y_prede_np], axis=1)
